...
<c:choose>
	...
	<c:otherwise>
	  <legend>新闻管理</legend>  
	 	<form class="form-search" id="submitForm" action="${pageContext.request.contextPath }/news_findNews.action" method="post" >
      ...
      <c:choose>
       <c:when test="${ empty uid }">
        <a href="${pageContext.request.contextPath }/news_edit?nid=${bean.nid}&hid=${hid}">编辑</a>
         <a href="${pageContext.request.contextPath }/news_delete?nid=${bean.nid}&hid=${hid}">删除</a>
        <a href="${pageContext.request.contextPath }/comment_findByPage?nid=${bean.nid}&&uid=${uid}">查看评论</a>
        <a href="${pageContext.request.contextPath }/news_addUI?nid=${bean.nid}&hid=${hid}">返回添加新闻</a>
      </c:when>
      <c:otherwise>
       <a href="${pageContext.request.contextPath }/comment_addUI?hid=${hid}&&uid=${uid}&&nid=${bean.nid}">添加评论</a>
        <a href="${pageContext.request.contextPath }/comment_findByPage?nid=${bean.nid}&&uid=${uid}">查看评论</a>
      </c:otherwise>
       </c:choose>
...
<div class="ui_frt">
	...
	[<A href="${pageContext.request.contextPath }/news_findByPage?hid=${hid}&pageCode=${pb.pageCode-1}">上一页</A>]
	</c:if>
	<c:if test="${pb.pageCode < pb.totalPage}">
	[<A href="${pageContext.request.contextPath }/news_findByPage?hid=${hid}&pageCode=${pb.pageCode+1}">下一页</A>]
	</c:if>
</div>
